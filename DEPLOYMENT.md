...
准备 Linux 服务器，将实际域名的 A/AAAA 记录指向服务器，开放 80/TCP、443/TCP；443/UDP 可选。若配置 AAAA，IPv6 也必须可达。默认配置要求域名直接解析到服务器，使用 Cloudflare DNS 时先关闭 CDN 代理。
...
脚本依次校验配置、构建镜像、下载国家 IP 地址库、启动服务、等待容器健康和公网 HTTPS，再打印一次性管理员初始化链接。Caddy 自动申请证书，没有默认账号或密码。打开初始化链接设置管理员后，该链接立即失效。
...
所有网页、登录、管理、API、普通视频与分享视频统一检查来源 IP。地址库国家代码 `CN` 返回 HTTP 403；港澳台不在屏蔽范围内。有效账号和免登录分享链接都不能跳过检查。IPv4、IPv6 和 IPv4 映射地址均检查。
...
将示例地址替换为实际地址。容器中的 `127.0.0.1` / `localhost` 是容器自身，不能作为宿主机代理。服务器需要自己的出口配置；脚本不复制本机 v2rayN 凭据。代理影响后端 API 和视频请求，封面仍使用观众浏览器的网络。
...
当前单 web 进程、一个视频准备名额，同一集共享下载和转换；不支持直接增加多 worker。首次准备和已缓存播放的耗时不同，承载能力取决于服务器带宽、磁盘和码率。
...
尚无实际服务器或域名验收。上线后用真实大陆网络确认返回 403，用海外网络检查初始化、登录、匿名分享、撤销，以及同一集首次播放、缓存播放、拖动和切集。单机代理或模拟来源地址不能代替真实地区客户端验收。

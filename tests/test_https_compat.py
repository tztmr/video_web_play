...
class HttpsCompatTests(unittest.TestCase):
    ...
    def test_caddyfiles_disable_http01_or_http3_in_compat_modes(self):
        ...
        self.assertIn('sh -s email="taco@${DOMAIN}" --home "$ACME_HOME" --noprofile >&2', stack)
        self.assertNotIn('sh -s -- --install-online', stack)
        self.assertIn(r"正在安装 acme.sh 到部署目录（不会改动 3x-ui）…\n' >&2", stack)
